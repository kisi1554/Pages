...
/*
 * 音まわり(日本地理たんけんの音エンジンを、すごろく用に効果音を足して流用)
...
const SoundEngine = (function createAudio() {
  ...
  // サイコロを ふる音（木のカップの中で ころころ 鳴る感じ）
  function seDice() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    for (let i = 0; i < 7; i += 1) {
      const at = t + i * 0.055 + Math.random() * 0.015;
      const src = ctx.createBufferSource();
  ...
  function seDiceStop() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    blip(660, t, 0.09, 'triangle', 0.28);
    blip(990, t + 0.07, 0.14, 'triangle', 0.24);
  }

  // 1マス すすむ音
  function seStep() {
    if (!seOn || !ensureCtx()) return;
    blip(1180, ctx.currentTime, 0.05, 'sine', 0.2);
  }
  ...
  function seCoin() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    [1318, 1760, 2093].forEach((hz, i) => blip(hz, t + i * 0.06, 0.14, 'square', 0.2));
  }
  ...
  function seLose() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    [660, 550, 440].forEach((hz, i) => blip(hz, t + i * 0.09, 0.16, 'triangle', 0.24));
  }
  ...
  function seCard() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    [880, 1174, 1568, 2093].forEach((hz, i) => {
      blip(hz, t + i * 0.05, 0.22, 'sine', 0.22);
      blip(hz * 2, t + i * 0.05, 0.12, 'triangle', 0.08);
    });
  }
  ...
  function seRest() {
    if (!seOn || !ensureCtx()) return;
    const t = ctx.currentTime;
    blip(392, t, 0.3, 'sine', 0.22);
    blip(294, t + 0.18, 0.4, 'sine', 0.2);
  }
